    ...
    double lower_bound)
{
    ...
    const bool has_a_log = a_log.has_value() && a_log->defined();
    const bool has_dt_bias = dt_bias.has_value() && dt_bias->defined();
    if (use_gate_in_kernel) {
        TORCH_CHECK(has_a_log,
                    "recurrent_kda: A_log is required when use_gate_in_kernel=True.");
        TORCH_CHECK(a_log->scalar_type() == at::kFloat && a_log->dim() == 1 &&
                        a_log->numel() == hv,
                    "recurrent_kda: A_log must be float32 [HV].");
        TORCH_CHECK(a_log->device() == query.device(),
                    "recurrent_kda: A_log must be on the same device as query.");
        if (has_dt_bias) {
            TORCH_CHECK(dt_bias->scalar_type() == at::kFloat &&
                            ((dt_bias->dim() == 1 &&
                              dt_bias->numel() == hv * k_dim) ||
                             (dt_bias->dim() == 2 &&
                              dt_bias->size(0) == hv &&
                              dt_bias->size(1) == k_dim)),
                        "recurrent_kda: dt_bias must be float32 [HV*K] or [HV,K].");
            TORCH_CHECK(dt_bias->device() == query.device(),
                        "recurrent_kda: dt_bias must be on the same device as query.");
        }
    } else {
        TORCH_CHECK(!has_a_log && !has_dt_bias && !safe_gate,
                    "recurrent_kda: A_log, dt_bias and safe_gate require "
