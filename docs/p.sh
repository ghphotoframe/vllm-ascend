...
# Bailing 3.0 Flash - PD disaggregated prefill, 1P1D on one machine, TP8.
# Usage:
#   bash bailing3.0_flash_start_prefill_1p1d_tp8.sh
#   PREFILL_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash bailing3.0_flash_start_prefill_1p1d_tp8.sh
...
# ----------------------------- model / ports ----------------------------- #
export MODEL_PATH="${MODEL_PATH:-/a3_inference/itask/workdir/glf02305252/bailing/bailing30-15/tensor-2}"
export CHAT_TEMPLATE="${CHAT_TEMPLATE:-${MODEL_PATH}/chat_template.jinja}"
export PREFILL_PORT="${PREFILL_PORT:-8100}"
export PREFILL_KV_PORT="${PREFILL_KV_PORT:-30000}"
export SERVER_HOST="${SERVER_HOST:-0.0.0.0}"
export PYTORCH_NPU_ALLOC_CONF=expandable_segments:True

# A TP8 prefill instance needs 8 visible NPUs. For same-machine 1P1D with both
# sides TP8, prefill and decode must use non-overlapping device lists.
export PREFILL_VISIBLE_DEVICES="${PREFILL_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}"
export ASCEND_RT_VISIBLE_DEVICES="${PREFILL_VISIBLE_DEVICES}"
...
export ASCEND_PROCESS_LOG_PATH="${LOG_PATH}/bailing_flash_prefill/ascend"
export MC_LOG_DIR="${LOG_PATH}/bailing_flash_prefill/mooncake"
rm -rf "${ASCEND_PROCESS_LOG_PATH}" "${MC_LOG_DIR}" 2>/dev/null || true
mkdir -p "${ASCEND_PROCESS_LOG_PATH}" "${MC_LOG_DIR}"

export VLLM_TORCH_PROFILER_DIR="${VLLM_TORCH_PROFILER_DIR:-/tmp/theta_workdir/profiles_bailing_flash_prefill}"
export VLLM_WORKDIR="${VLLM_WORKDIR:-/tmp/theta_workdir/bailing_flash_prefill}"
...
kv_transfer_config=$(cat <<EOF
{
    ...
    "engine_id": "bailing-flash-prefill-0",
    "kv_rank": 0,
...
echo "Starting Bailing 3.0 Flash prefill:"
echo "  MODEL_PATH=${MODEL_PATH}"
echo "  ASCEND_RT_VISIBLE_DEVICES=${ASCEND_RT_VISIBLE_DEVICES}"
echo "  HTTP port=${PREFILL_PORT}, KV port=${PREFILL_KV_PORT}"
echo "  log=${LOG_PATH}/vllm_bailing_flash_prefill.log"
