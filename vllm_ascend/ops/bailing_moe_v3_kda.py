...
class AscendBailingMoeV3KimiDeltaAttention(BailingMoeV3KimiDeltaAttention):
    ...
    def _forward(
        ...
        if attn_metadata.num_prefills > 0:
            ...
            if conv_state_for_op.dtype != qkv.dtype:
                conv_state_for_op = conv_state_for_op.to(dtype=qkv.dtype)
            ...
            torch.ops._C_ascend.npu_causal_conv1d_custom(
                ...
                cache_indices_opt=state_indices,
            ...
            # Pack q/k/v and conv weights (same as prefill path)
            qkv = torch.cat(
                (
                    q_proj_states_non_spec,
                    k_proj_states_non_spec,
            ...
            conv_state_for_op = conv_state_storage
            if is_conv_state_dim_first():
                conv_state_for_op = conv_state_for_op.transpose(
                    -1, -2
                ).contiguous()
            if conv_state_for_op.dtype != qkv.dtype:
                conv_state_for_op = conv_state_for_op.to(dtype=qkv.dtype)

            # Call C operator with run_mode=1 (decode mode)
            qkv_out = torch.empty_like(qkv)
            torch.ops._C_ascend.npu_causal_conv1d_custom(
                qkv_out,
                qkv,
                ...
                bias_opt=None,
                query_start_loc_opt=query_start_loc,
                cache_indices_opt=decode_indices,
            ...
            # Write back conv_state if needed (same as prefill path)
            if conv_state_for_op is not conv_state_storage:
                if is_conv_state_dim_first():
                    conv_state_for_op = conv_state_for_op.transpose(-1, -2)
                conv_state_storage.copy_(
