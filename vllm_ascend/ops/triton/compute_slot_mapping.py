...
def _compute_slot_mapping_kernel(
    ...
    for i in range(start_idx, end_idx, TILE_BLOCK_SIZE):
        ...
        # Cast to int32 immediately to avoid int64 scalar computation on NPU.
        ...
        # Load a contiguous window of the block table and gather the needed
        # entries. This avoids scattered (non-contiguous) loads that would
        # degrade to scalar computation on Ascend NPUs.
        #
        # Constraint: block numbers stored in float32 during gather must
        # satisfy abs(block_id) <= 2^24 (16M blocks). This holds for all
        # practical model/sequence-length configurations.
